...
import MesCW500TopNavRightSideNavItem from './MesCW500TopNavRightSideNavItem';

const MesCW500NavbarTop = () => {
  const CheckDev = useIsMobile();

  const {
    config: { naverp, showBurgerMenu, navbarPosition, navbarCollapsed },
    setConfig
  ...
  return (
    <Navbar
      className={classNames(CheckDev.isMobile ? 'navbar-glass-mobile fs--1 pt-0 pb-0 navbar-top sticky-kit' : 'navbar-glass fs--1 pt-0 pb-0 navbar-imes bg-light shadow navbar-top sticky-kit', {
      ...
      {navbarPosition === 'double-top' ? (
        <div className="w-100">
          <div className="d-flex flex-between-center">
            <NavbarTopElements navbarCollapsed={navbarCollapsed} navbarPosition={navbarPosition} handleBurgerMenu={handleBurgerMenu} burgerMenuRef={burgerMenuRef} />
          </div>
...
const NavbarTopElements = ({ navbarPosition, handleBurgerMenu, navbarCollapsed }) => {
  ...
  return (
    <>
      ...
      {CheckDev.isMobile ? <Logo at="navbar-top" width={150} id="topLogo" /> : <LogoDesk at="navbar-top" width={300} id="topLogo" />}
